import requests
from flask import Flask, render_template, request, redirect
import psycopg2

app = Flask(__name__)

conn = psycopg2.connect(database="service_db",
                        user="postgres",
                        password="3478",
                        host="localhost",
                        port="5432")

cursor = conn.cursor()


@app.route('/login/', methods=['POST', 'GET'])
def login():
    if request.method == 'POST':
        if request.form.get("log"):
            login = request.form.get('login')
            password = request.form.get('password')
            cursor.execute("SELECT * FROM service.users WHERE login=%s AND password=%s", (str(login), str(password)))
            records = list(cursor.fetchall())
            if login and password:
                try:
                    return render_template('account.html', full_name=records[0][1], login=login, password=password)
                except:
                    wrong_data = "Неверный логин или пароль!"
                    return render_template('login.html', wrong_data=wrong_data)
            empty_data = "Введите логин и пароль"
            return render_template('login.html', empty_data=empty_data)
        elif request.form.get("registration"):
            return redirect("/registration/")
    return render_template('login.html')


@app.route('/registration/', methods=['POST', 'GET'])
def registration():
    if request.method == 'POST':
        name = request.form.get('name')
        login = request.form.get('login')
        password = request.form.get('password')

        if name and login and password:
            cursor.execute("SELECT * FROM service.users WHERE login=%s", (str(login),))
            records = list(cursor.fetchall())
            try:
                rec = records[0][1]
                user_exists = 'Пользователь с таким логином уже существует!'
                return render_template('registration.html', user_exists=user_exists)
            except:

                cursor.execute('INSERT INTO service.users (full_name, login, password) VALUES (%s, %s, %s);',
                               (str(name), str(login), str(password)))
                conn.commit()

                return redirect('/login/')
        empty_data= 'Заполните все поля!'
        return render_template('registration.html', empty_data=empty_data)

    return render_template('registration.html')
